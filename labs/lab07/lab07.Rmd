...
# If you use dweeks, the hour changes! This is because there is a daylight savings time change between these lab dates, so November 6th has 25 hours instead of 24. In this case, this behavior is undesirable because we would expect that the time class start doesn't change. This behavior is more useful in situations where the exact interval of time between observations is important, like taking scientific measurements of something every hour over a long period of time. 
```
...
```{r cleanHuron, exercise=TRUE, exercise.blanks = "\\.\\.\\.+"}
...
**Exercise**: Finally, let's plot the data over time.

```{r plotHuron, exercise=TRUE}
