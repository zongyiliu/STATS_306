...
dots(.) are not recommended for variable names, we want to rename all columns names in the iris dataset but displacing . with _
...
`summarise` is used to compute a new dataset (tibble) by using agrregation functions. It is usually useful when used with `group_by`, as this helps to compute summaries for each category of a column of a dataset.
