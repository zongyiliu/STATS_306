...
A `ggplot2` plot consists of a base created by `ggplot()` and zero or more layers, each representing observations in a particular dataset using a particular geom. There are many geoms; several commonly used geoms are described below.
...
If the variable is categorical, then we can make a bar chart. We use `geom_bar()` to do this. The plot below shows the number of counties for various midwestern states.
