...
Filtering out the players with `NA`s for `debut_d` or `finalGame_d`. Which player has had the longest career? (Display the name along with the duration in years.)
...
Look up this player on Wikipedia to learn how he had such a long career. Was he playing the entire time? With which other player did he perform comedy routines?
...
group_by(Fielding, playerID, yearID, teamID) |> summarize(teams = n()) |> ungroup() |> summarize(sum(teams > 1))
group_by(Fielding, playerID, yearID, teamID, POS) |> summarize(positions = n()) |> ungroup() |> summarize(sum(positions > 1))
```
## Q 3.2
...
Use a `left_join` to determine how many managers won an award in each year. Graph this as a line plot. What do you notice about the proportion of managers winning awards?
